 ...
 */
public final class ProcessIpStream {
  ...
  public static void main(final String[] args) throws RuntimeException {
    ...
    final ProcessIpStream pips = new ProcessIpStream();
  ...
  private void processIpStream(final int initialNumEntries) {
    ...
    final UniqueCountMap map = new UniqueCountMap(initialNumEntries, IP_BYTES);
    ...
    try (BufferedReader br = new BufferedReader(new InputStreamReader(System.in, UTF_8))) {
      ...
      while ((line = br.readLine()) != null) {
        ...
        checkLen(tokens);
        lineCount++;
        final byte[] iAddBytes = InetAddress.getByName(tokens[0]).getAddress();
        final byte[] valBytes = tokens[1].getBytes(UTF_8);
      ...
      final int ipCount = map.getActiveEntries();
  ...
  static void printStats(final StringBuilder sb, final String className, final UniqueCountMap map,
      final long lineCount, final int ipCount, final long updateCount, final long updateTime_nS) {
  ...
  static void printTaskTime(final StringBuilder sb, final long total_mS, final long updateCount) {
  ...
  static final void checkLen(final String[] tokens) {
