...
package com.yahoo.sketches.misc.druidhll;
...
public abstract class HyperLogLogCollector implements Comparable<HyperLogLogCollector>
{
  ...
  )
  {
    ...
    while (copy.hasRemaining()) {
      ...
      final int register = copy.get() & 0xff;
  ...
  public void add(byte[] hashedValue)
  {
    if (hashedValue.length < minBytesRequired) {
      throw new IllegalArgumentException(
          "Insufficient bytes, need[%d] got [%d] " + minBytesRequired +", " + hashedValue.length);
    }
    ...
    for (int i = 0; i < 8; ++i) {
      byte lookupVal = ByteBitLookup.lookup[hashedValue[i] & 0XFF];
      switch (lookupVal) {
        case 0:
          positionOf1 += (byte) 8;
          continue;
  ...
  public HyperLogLogCollector fold(HyperLogLogCollector other)
  {
    ...
    try {
      ...
      if (offsetDiff < 0) {
        throw new IllegalStateException("offsetDiff[%d] < 0, shouldn't happen because of swap. " + offsetDiff);
      }
  ...
  private short addNibbleRegister(short bucket, byte positionOf1)
  {
    ...
    storageBuffer.put(
        position,
        (byte) (max((byte) (origVal & newValueMask), shiftedPositionOf1) | (origVal & originalValueMask))
    );
